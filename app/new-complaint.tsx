...
export default function NewComplaintPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.back();
    } catch (error) { showAlert('Error', 'Failed to submit complaint', [], 'error'); } finally { setLoading(false); }
  };
      ...
      <KeyboardAvoidingView behavior="padding" keyboardVerticalOffset={Platform.OS === 'ios' ? 0 : 20} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={[styles.content, { paddingBottom: 80 }]} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={textMain} />}>
          ...
          <View style={styles.formContainer}>
            ...
            <View style={styles.inputGroup}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.priorityContainer}>
                {(['low', 'medium', 'high', 'emergency'] as const).map((level) => (
                  <Pressable key={level} style={[styles.priorityButton, { backgroundColor: chipBg, borderColor: inputBorder }, priority === level && [styles.prioritySelected, { backgroundColor: chipSelectedBg, borderColor: chipSelectedBg }]]} onPress={() => setPriority(level)}>
            ...
            <Pressable onPress={handleSubmit} style={({ pressed }) => [styles.submitButton, { backgroundColor: primaryBtnBg }, (!title || !description || loading) && { opacity: 0.5 }, pressed && { opacity: 0.8 }]} disabled={!title || !description || loading}>
                {loading ? <ActivityIndicator color={primaryBtnText} /> : <AppText style={[styles.buttonText, { color: primaryBtnText }]}>SUBMIT COMPLAINT</AppText>}
            </Pressable>
          </View>
        </ScrollView>
